...
//            关闭连接
    ...
    private static void putData(Connection con,List<Put> bankList) throws Exception {
        Table table = con.getTable(TableName.valueOf("five_day_top"));
//        put数据
        table.put(bankList);
//        关闭
        table.close();
    ...
    private static void createTable(Connection con) throws Exception{
        HTableDescriptor td = new HTableDescriptor(TableName.valueOf("five_day_top"));
        HColumnDescriptor cd = new HColumnDescriptor("cf01");
        td.addFamily(cd);
        Admin admin = con.getAdmin();
        if (!admin.tableExists(TableName.valueOf("five_day_top"))){
