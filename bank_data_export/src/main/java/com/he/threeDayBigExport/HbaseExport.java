package com.he.threeDayBigExport;
...
//            关闭连接
    ...
    private static void putData(Connection con,List<Put> bankList) throws Exception {
        Table table = con.getTable(TableName.valueOf("three_day_big"));
//        put数据
        table.put(bankList);
//        关闭
        table.close();
    ...
    private static void createTable(Connection con) throws Exception{
        HTableDescriptor td = new HTableDescriptor(TableName.valueOf("three_day_big"));
        HColumnDescriptor cd = new HColumnDescriptor("cf01");
        td.addFamily(cd);
        Admin admin = con.getAdmin();
        if (!admin.tableExists(TableName.valueOf("three_day_big"))){
